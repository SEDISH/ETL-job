...
if [ "$DB" != "isanteplus" ]; then
  ...
  mysql -u root -p${MYSQL_ROOT_PASSWORD} -h ${MYSQL_HOST} -D isanteplus -e "CALL isanteplusreports_dml();"
  mysql -u root -p${MYSQL_ROOT_PASSWORD} -h ${MYSQL_HOST} -D isanteplus -e "CALL patient_status_arv();"
fi
